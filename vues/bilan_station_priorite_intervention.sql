...
CREATE VIEW bilan.bilan_station_prospection AS

WITH bilan_station_lal AS (
SELECT bslal.id_station,'Leucorrhinia albifrons (Burmeister, 1839)' AS nom_latin,bslal.classement_station, bslal.derniere_annee_total AS date_derniere_obs,
...
bilan_lal AS (
SELECT ROW_NUMBER() over()::bigint as gid,bilan_station_lal.id_station,nom_latin,classement_station,date_derniere_obs,
       extract(year FROM date_derniere_prosp) AS annee_derniere_prosp,extract(year FROM date_non_obs) AS annee_non_obs,
       CASE
           WHEN GREATEST (date_derniere_obs,extract(year FROM date_derniere_prosp),extract(year FROM date_non_obs))::integer >= (2021 - 4) THEN 'Prospection récente'::text
           WHEN GREATEST (date_derniere_obs,extract(year FROM date_derniere_prosp),extract(year FROM date_non_obs))::integer >= (2021 - 8) THEN 'Prospection ancienne'::text
           WHEN GREATEST (date_derniere_obs,extract(year FROM date_derniere_prosp),extract(year FROM date_non_obs))::integer < (2021 - 8) THEN 'Prospection historique'::text
...
SELECT bslca.id_station,'Leucorrhinia caudalis (Charpentier, 1840)' AS nom_latin,bslca.classement_station, bslca.derniere_annee_total AS date_derniere_obs,
       station.date_derniere_prosp,station.geom
...
bilan_lca AS (
SELECT ROW_NUMBER() over()::bigint + 1000 as gid,bilan_station_lca.id_station,nom_latin,classement_station,date_derniere_obs,
       extract(year FROM date_derniere_prosp) AS annee_derniere_prosp,extract(year FROM date_non_obs) AS annee_non_obs,
       CASE
           WHEN GREATEST (date_derniere_obs,extract(year FROM date_derniere_prosp),extract(year FROM date_non_obs))::integer >= (2021 - 4) THEN 'Prospection récente'::text
           WHEN GREATEST (date_derniere_obs,extract(year FROM date_derniere_prosp),extract(year FROM date_non_obs))::integer >= (2021 - 8) THEN 'Prospection ancienne'::text
...
SELECT bslpe.id_station,'Leucorrhinia pectoralis (Charpentier, 1825)' AS nom_latin,bslpe.classement_station, bslpe.derniere_annee_total AS date_derniere_obs,
       station.date_derniere_prosp,station.geom
...
bilan_lpe AS (
SELECT ROW_NUMBER() over()::bigint + 5000 as gid,bilan_station_lpe.id_station,nom_latin,classement_station,date_derniere_obs,
       extract(year FROM date_derniere_prosp) AS annee_derniere_prosp,extract(year FROM date_non_obs) AS annee_non_obs,
       CASE
           WHEN GREATEST (date_derniere_obs,extract(year FROM date_derniere_prosp),extract(year FROM date_non_obs))::integer >= (2021 - 4) THEN 'Prospection récente'::text
           WHEN GREATEST (date_derniere_obs,extract(year FROM date_derniere_prosp),extract(year FROM date_non_obs))::integer >= (2021 - 8) THEN 'Prospection ancienne'::text
